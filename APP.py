...
if 'num_especimes_anterior_comp' not in st.session_state: # Para inputs experimentais
    st.session_state.num_especimes_anterior_comp = 0
if 'num_medicoes_exp_anterior' not in st.session_state: # Para inputs experimentais
    st.session_state.num_medicoes_exp_anterior = 0
...
with left_column:
    ...
    with st.expander("🌿 2. Dados Biométricos e Estruturais do Espécime", expanded=True):
        ...
        if 'num_especimes_anterior_bio' not in st.session_state or st.session_state.num_especimes_anterior_bio != num_especimes_bio:
            st.session_state.especimes_data_list = [{}] * num_especimes_bio # Reinicia com dicts vazios
            st.session_state.num_especimes_anterior_bio = num_especimes_bio
        
        for i in range(num_especimes_bio):
            # Garante que o dict existe e tem chaves padrão
            if i >= len(st.session_state.especimes_data_list) or not isinstance(st.session_state.especimes_data_list[i], dict):
                st.session_state.especimes_data_list.append({}) # Adiciona se não existe
            ...
            if len(folhas_data_temp) != num_folhas_amostra: # Ajusta se o número de amostras mudou
                folhas_data_temp = [("6","12")] * num_folhas_amostra
            ...
            for j in range(num_folhas_amostra):
                with cols_folhas[j]:
                    st.markdown(f"🍃 F{j+1}")
                    val_l, val_c = folhas_data_temp[j]
    ...
    with st.expander("🛠️ 4. Coeficientes do Modelo (Demonstrativo/Ajustável)", expanded=False):
        # ... (código dos coeficientes permanece o mesmo) ...
        ...
        st.session_state.et_model_coeffs["altura"] = cols_coeffs_et1[0].number_input("Peso Altura:", value=st.session_state.et_model_coeffs["altura"], step=0.01, format="%.2f", key="coeff_alt_cfg")
        st.session_state.et_model_coeffs["diametro"] = cols_coeffs_et1[1].number_input("Peso Diâmetro:", value=st.session_state.et_model_coeffs["diametro"], step=0.01, format="%.2f", key="coeff_diam_cfg")
        cols_coeffs_et2 = st.columns(2)
        st.session_state.et_model_coeffs["copa"] = cols_coeffs_et2[0].number_input("Peso Área Copa:", value=st.session_state.et_model_coeffs["copa"], step=0.01, format="%.2f", key="coeff_copa_cfg")
        st.session_state.et_model_coeffs["lai"] = cols_coeffs_et2[1].number_input("Peso LAI:", value=st.session_state.et_model_coeffs["lai"], step=0.01, format="%.2f", key="coeff_lai_cfg")
        cols_coeffs_et3 = st.columns(2)
        st.session_state.et_model_coeffs["temperatura"] = cols_coeffs_et3[0].number_input("Peso Temperatura:", value=st.session_state.et_model_coeffs["temperatura"], step=0.01, format="%.2f", key="coeff_temp_cfg")
        st.session_state.et_model_coeffs["umidade"] = cols_coeffs_et3[1].number_input("Peso Umidade:", value=st.session_state.et_model_coeffs["umidade"], step=0.01, format="%.3f", key="coeff_umid_cfg")
        cols_coeffs_et4 = st.columns(2)
        st.session_state.et_model_coeffs["radiacao"] = cols_coeffs_et4[0].number_input("Peso Radiação:", value=st.session_state.et_model_coeffs["radiacao"], step=0.001, format="%.3f", key="coeff_rad_cfg")
        st.session_state.et_model_coeffs["vento"] = cols_coeffs_et4[1].number_input("Peso Vento:", value=st.session_state.et_model_coeffs["vento"], step=0.001, format="%.3f", key="coeff_vento_cfg")
    ...
    if st.button("🚀 Executar Simulação e Análise", type="primary", key="run_simulation_main_button", use_container_width=True):
        ...
        for i, especime_input_data in enumerate(st.session_state.especimes_data_list):
            if not especime_input_data: # Checa se o dict está vazio
                st.warning(f"Dados para o Espécime {i+1} não foram completamente preenchidos na interface. Pulando.")
                continue
            try:
...
with right_column:
    ...
    if not st.session_state.resultados_modelo:
        ...
        # ... (código dos gráficos permanece o mesmo, já estava ok) ...
        ...
        num_medicoes_exp_atual = st.number_input("Número de Medições Experimentais por Espécime:", min_value=1, value=st.session_state.get('num_medicoes_exp_anterior', 3), step=1, key="num_med_exp_comp_input")
        tempo_coleta_exp_horas_str = st.text_input("Tempo de Coleta para Cada Medição Experimental (horas):", st.session_state.get('tempo_coleta_exp_horas_str_anterior', "24"), key="tempo_coleta_exp_h_comp_input")

        # Re-inicializa ou ajusta st.session_state.contraprovas_data se o número de espécimes ou medições mudou
        if st.session_state.num_especimes_anterior_comp != df_resultados.shape[0] or \
           st.session_state.num_medicoes_exp_anterior != num_medicoes_exp_atual:
            st.session_state.contraprovas_data = {} # Limpa para reconstruir
        ...
        st.session_state['tempo_coleta_exp_horas_str_anterior'] = tempo_coleta_exp_horas_str # Salva para persistência

        for _, row_modelo in df_resultados.iterrows(): # Loop para criar os campos de input
            id_especime_modelo = row_modelo["ID Espécime"]
            # Garante que a entrada existe no session_state com o tamanho correto
                ...
                for k in range(num_medicoes_exp_atual):
                    with cols_med_exp_display[k]:
                        val_exp_input = st.text_input(
                            f"Medição {k+1} (mL):",
                            value=st.session_state.contraprovas_data[id_especime_modelo][k],
                            key=f"med_exp_val_{id_especime_modelo}_{k}"
                        )
                        medicoes_atuais_str.append(val_exp_input)
                st.session_state.contraprovas_data[id_especime_modelo] = medicoes_atuais_str # Atualiza o estado com os valores dos inputs
        ...
        if st.button("🔄 Comparar Modelo com Dados Experimentais", key="run_comparison_main_button", use_container_width=True):
            ...
            for _, row_modelo in df_resultados.iterrows():
                ...
                for idx_med, med_str_val in enumerate(medicoes_exp_str_list):
                    try:
                        ...
                        if not med_str_cleaned:
                            st.warning(f"Medição {idx_med+1} (Espécime {id_especime}) está vazia. Ignorando.")
                            continue
                        med_float = float(med_str_cleaned)
                        medicoes_exp_ml_float_validas.append(med_float)
                    except ValueError:
                        st.error(f"Valor experimental '{med_str_val}' (Medição {idx_med+1}, Espécime {id_especime}) não é um número válido.")
                        valores_exp_invalidos_flag = True; break 
                
                if valores_exp_invalidos_flag:
                    st.warning(f"Análise para Espécime {id_especime} interrompida devido a valores experimentais inválidos."); continue
                if not medicoes_exp_ml_float_validas:
                    st.warning(f"Nenhum dado experimental válido para Espécime {id_especime}. Pulando."); continue
                ...
                p_valor_teste_atual = None # Lógica dos testes estatísticos ...
                # ... (código dos testes estatísticos, já corrigido anteriormente, permanece aqui) ...
                if tipo_teste_estatistico == "Teste t de Student (1 amostra)":
                    if len(medicoes_exp_L_dia) < 2 or len(set(medicoes_exp_L_dia)) < 2 : 
            ...
            if valid_comparison_count > 1: # Análise Global
                ...
                # ... (código da análise global, já corrigido anteriormente, permanece aqui) ...
...
# ---------------------------------------------------------------
# Seções Explicativas e de Discussão (Nível PhD) - LaTeX corrigido
# ---------------------------------------------------------------
...
with st.expander("🔍 Fundamentos Teóricos e Metodológicos (Discussão para Banca)", expanded=False):
    ...
    st.markdown(r"""... $ET_0 = \frac{0.408 \Delta (R_n - G) + \gamma \frac{900}{T+273} u_2 (e_s - e_a)}{\Delta + \gamma (1 + 0.34 u_2)}$ ...""")
    st.markdown(r"### 🍂 Área Foliar Total (AFT) e Índice de Área Foliar (LAI)")
    st.markdown(r"""...""")
    st.markdown(r"### 🌳 Estimativa de Absorção/Captura de Carbono")
    st.markdown(r"""... $A = \min(A_c, A_j, A_p) - R_d$ ... $NPP = APAR \times LUE_{eco}$ ... $WUE = A / E$ ...""")
    ...
    st.markdown(r"""... $f(x; q, \beta) = N \exp_q(-\beta x) = N [1 - (1-q) \beta x]_+^{1/(1-q)}$ ...""")
with st.expander("🌳 Estimativa Simplificada de Absorção/Captura de Carbono (Conceitual Detalhado)", expanded=False):
    ...
    st.markdown(r"""...onde \( k_{\text{AFT}} \) e \( k_{\text{ET/WUE}} \) ...""")
st.markdown("---")
